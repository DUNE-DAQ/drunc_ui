...
from drunc_ui.interfaces.process_manager_interface import boot_process, get_process_logs
...
def logs(request: HttpRequest, uuid: uuid.UUID) -> HttpResponse:
    ...
    Returns:
      The rendered page.
    """
    log_lines = get_process_logs(str(uuid), request.user.username)
...
class BootProcessView(PermissionRequiredMixin, FormView[BootProcessForm]):
    ...
    def form_valid(self, form: BootProcessForm) -> HttpResponse:
        ...
        """
        boot_process(self.request.user.username, *form.cleaned_data.values())
