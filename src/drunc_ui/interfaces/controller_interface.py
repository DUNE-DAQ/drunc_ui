...
def get_controller_uri() -> str:
    ...
    try:
        ...
        raise ApplicationLookupUnsuccessful("Unable to connect with the Root Controller") from e
    if len(uris) != 1:
        raise ValueError(f"Expected 1 URI for root-controller, found {len(uris)}: {uris}")
...
def send_event(  # type: ignore[explicit-any]
    ...
    command = FSMCommand(command_name=event, arguments=process_arguments(event, arguments))
